...
GCS_KEY_FILE = "key-2.json"
GCS_BUCKET_NAME = "q2-0c825af4b282f9f-logs"
...
def call_gemini(contents, system_instruction=None, retries=5):
    ...
    for attempt in range(retries):
        try:
            ...
            # If rate limited (429), back off significantly longer to allow the rate limit bucket to reset
            if "429" in str(e):
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i in range(max_iterations):
        ...
        format_instruction = (
            "You are a formatter. You must output the final answer to the user's question as a raw JSON object matching the exact shape "
            "the user requested. Do NOT write any python code blocks. Do NOT write explanations. Output ONLY the JSON."
        )
        try:
            final_response = call_gemini(contents, format_instruction)
    ...
    if render_url:
        log_url = f"{render_url.rstrip('/')}/{LOG_FILENAME}"
    else:
